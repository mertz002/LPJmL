    ...
    for (i = 0; i < config->ngridcell; i++)
    {
        ...
        if (config->river_routing)
        {
            if (config->lakes_filename.fmt == CDF)
            {
                ...
                grid[i].lakefrac /= 100; // hb because the input is a int, fraction needed
            }
            else
            {
        ...
        if (file_restart == NULL)
        {
            ...
            else
            {
                ...
                if (grid[i].lakefrac < 1)
                {
                    ...
                    if (initsoil(&stand->soil, config->soilpar + soilcode - 1, npft + ncft))
                        return NULL;
        ...
        if (!grid[i].skip)
        {
            ...
            if (i % 1000 == 0)
                printf("initialized product pools in pix %d to %g %g\n", i,
